...
module.exports = (from, to, answer) => {
	...
	// const packageJsonStream = fs.createWriteStream("package.json");

	// packageJsonStream.write(JSON.stringify(goodPackageJson, null, 2), err => {
	// 	if (err) throw err;
	// });

	webpackConfigStream.write(`module.exports = {${JSON.parse(goodWebpackJson, null, 2)}}`, err => {
		if (err) throw err;
	});
